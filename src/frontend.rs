...
use crate::{converter::Converter, wrapper::SearchIndexWrapper};
...
/// the text.
pub struct FMIndex<T: Copy + Clone, C: Converter<Char = T>>(
    SearchIndexWrapper<FMIndexBackend<T, C, ()>>,
);
/// Search result for FMIndex, count only.
pub struct FMIndexSearch<'a, T: Copy + Clone, C: Converter<Char = T>>(
    SearchWrapper<'a, FMIndexBackend<T, C, ()>>,
);
/// Match in the text for FMIndex.
pub struct FMIndexMatch<'a, T: Copy + Clone, C: Converter<Char = T>>(
...
pub struct RLFMIndex<T: Copy + Clone, C: Converter<Char = T>>(
    SearchIndexWrapper<RLFMIndexBackend<T, C, ()>>,
);
/// Search result for RLFMIndex, count only.
pub struct RLFMIndexSearch<'a, T: Copy + Clone, C: Converter<Char = T>>(
    SearchWrapper<'a, RLFMIndexBackend<T, C, ()>>,
);
/// Match in the text for RLFMIndex.
pub struct RLFMIndexMatch<'a, T: Copy + Clone, C: Converter<Char = T>>(
...
pub struct MultiTextFMIndex<T: Copy + Clone, C: Converter<Char = T>>(
    SearchIndexWrapper<MultiTextFMIndexBackend<T, C, ()>>,
);
/// Search result for MultiText index, count only.
pub struct MultiTextFMIndexSearch<'a, T: Copy + Clone, C: Converter<Char = T>>(
    SearchWrapper<'a, MultiTextFMIndexBackend<T, C, ()>>,
);
/// Match in the text for MultiText index.
pub struct MultiTextFMIndexMatch<'a, T: Copy + Clone, C: Converter<Char = T>>(
...
/// This is a multi-text version of the FM-Index. It allows \0 separated strings.
/// It uses additional storage to support locate queries.
pub struct MultiTextFMIndexWithLocate<T: Copy + Clone, C: Converter<Char = T>>(
...
pub struct MultiTextFMIndexSearchWithLocate<'a, T: Copy + Clone, C: Converter<Char = T>>(
...
pub struct MultiTextFMIndexMatchWithLocate<'a, T: Copy + Clone, C: Converter<Char = T>>(
    MatchWrapper<'a, MultiTextFMIndexBackend<T, C, SuffixOrderSampledArray>>,
);

impl<T: Copy + Clone, C: Converter<Char = T>> FMIndex<T, C> {
...
impl<T: Copy + Clone, C: Converter<Char = T>> FMIndexWithLocate<T, C> {
...
impl<T: Copy + Clone, C: Converter<Char = T>> RLFMIndex<T, C> {
...
impl<T: Copy + Clone, C: Converter<Char = T>> RLFMIndexWithLocate<T, C> {
...
impl<T: Copy + Clone, C: Converter<Char = T>> MultiTextFMIndex<T, C> {
...
impl<T: Copy + Clone, C: Converter<Char = T>> MultiTextFMIndexWithLocate<T, C> {
...
macro_rules! impl_search_index {
    ($t:ty, $s:ident, $st:ty) => {
        impl<T: Copy + Clone, C: Converter<Char = T>> SearchIndex<T> for $t {
            fn search<K>(&self, pattern: K) -> impl Search<T>
            where
                K: AsRef<[T]>,
            {
        ...
        impl<T: Copy + Clone, C: Converter<Char = T>> HeapSize for $t {
            fn heap_size(&self) -> usize {
                self.0.heap_size()
            }
        }
        // inherent
        impl<T: Copy + Clone, C: Converter<Char = T>> $t {
            /// Search for a pattern in the text.
...
macro_rules! impl_search_index_with_multi_texts {
    ($t:ty, $s:ident, $st:ty) => {
        impl<T: Copy + Clone, C: Converter<Char = T>> SearchIndexWithMultiTexts<T> for $t {
            fn search_prefix<K>(&self, pattern: K) -> impl Search<T>
            where
                K: AsRef<[T]>,
            {
...
macro_rules! impl_search {
    ($t:ty, $m:ident, $mt:ty) => {
        impl<'a, T: Copy + Clone, C: Converter<Char = T>> Search<'a, T> for $t {
            type Match = $mt;
        ...
        // inherent
        impl<'a, T: Copy + Clone, C: Converter<Char = T>> $t {
            /// Search in the current search result, refining it.
            ///
            /// This adds a prefix `pattern` to the existing pattern, and
            /// looks for those expanded patterns in the text.
...
macro_rules! impl_match {
    ($t:ty) => {
        impl<'a, T: Copy + Clone, C: Converter<Char = T>> Match<'a, T> for $t {
            fn iter_chars_forward(&self) -> impl Iterator<Item = T> + 'a {
...
macro_rules! impl_match_locate {
    ($t:ty) => {
        impl<'a, T: Copy + Clone, C: Converter<Char = T>> MatchWithLocate<'a, T> for $t {
            fn locate(&self) -> usize {
                self.0.locate()
            }
        }
...
macro_rules! impl_match_text_id {
    ($t:ty) => {
        impl<'a, T: Copy + Clone, C: Converter<Char = T>> MatchWithTextId<'a, T> for $t {
            fn text_id(&self) -> TextId {
                self.0.text_id()
            }
        }
